...
//
// 1v1 - server autoritativo: questo canale porta ora SOLO dati cosmetici (posizione/rotazione
// della navicella avversaria e i suoi proiettili in volo, vedi webrtc/snapshot.ts) - l'handshake
// di inizio partita che viaggiava qui (matchInit: seed pseudo-casuale condiviso + istante di
// partenza comune, usato per far simulare ai due client la STESSA arena in locale) e' stato
// rimosso insieme alla simulazione locale che serviva: l'inizio partita e il seed dell'arena sono
// ora decisi dal server dell'arena (vedi arena-server/server.js, messaggio "matchStart") e non
// hanno piu' bisogno di attraversare questo canale peer-to-peer ne' di essere riallegati contro le
// perdite del canale inaffidabile (vedi la vecchia MATCH_INIT_RESEND_WINDOW_MS, anch'essa rimossa).
