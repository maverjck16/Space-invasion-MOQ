...
// Esempio: https://host/?auto=1&scenario=scenario-1&player=A&room=room-test-1
type AutoRunParams = {
  scenarioId: string;
  player: PlayerId;
  room: string;
...
// esistenti, vedi moq/subscriber.ts) prima di avviare lo scenario, cosi' i due client automatici
...
async function join(
  ...
  try {
    ...
    if (scenario && auto) {
      ...
      // proprio matchInit (vedi moq/publisher.ts, ignorato in modalita' automatica), e i due client
      // partirebbero da punti diversi della sequenza.
