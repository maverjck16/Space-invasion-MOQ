...
// a osservare i messaggi che passano già da publisher.ts/subscriber.ts.
//
...
// GameSnapshot resta invariato: publisher/subscriber lo impacchettano/spacchettano qui, il resto
...
// giocatore entra effettivamente in una room (dopo connectToRelay + startPublisher).
...
// messaggio pubblicato, per permettere al peer di calcolare l'RTT sul proprio orologio.
export function consumePendingEcho(): { seq: number; tSent: number } | undefined {
  return pendingEcho;
}

//  Da chiamare subito dopo aver scritto con successo un messaggio sulla rete (publisher.ts).
...
//  Da chiamare subito dopo aver letto con successo un messaggio dal peer remoto (subscriber.ts).
...
export function recordReceived(peer: string, envelope: NetEnvelope, bytes: number): void {
  ...
  if (state.lastSeq !== null) {
    if (envelope.seq > state.lastSeq + 1) {
      ...
      // Con lo scarto del backlog lato subscriber questo non dovrebbe mai accadere: utile come
      // controllo di correttezza. Conta sia veri duplicati sia arrivi fuori ordine.
...
export function exportJSON(): void {
  ...
  downloadFile(
    `moq-metrics-${sessionUsername}-${sessionRoom}-${timestampForFilename()}.json`,
...
// atteso da Excel in locale italiano, dove la virgola e' il separatore decimale).
export function exportCSV(): void {
  ...
  downloadFile(
    `moq-metrics-${sessionUsername}-${sessionRoom}-${timestampForFilename()}.csv`,
    [header, ...rows].join("\n"),
    "text/csv",
  );
}
