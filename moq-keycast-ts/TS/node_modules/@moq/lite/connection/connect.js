...
export async function connect(url, props) {
    ...
    const client = new Ietf.ClientSetup({
        ...
        versions: setupVersion === Ietf.Version.DRAFT_16
    ...
    if (Object.values(Lite.Version).includes(server.version)) {
        console.debug(url.toString(), "moq-lite session established");
        return new Lite.Connection(url, session, server.version, stream);
    }
    else if (Object.values(Ietf.Version).includes(server.version)) {
